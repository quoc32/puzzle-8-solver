...
import time
import numpy
from algorithm.AStar import aStar_and_generation_data, AStar
...
def create_frame(go_to, window, update_solver_path):
    ...
    frame.button_2 = Button(
        ...
        borderwidth=0,
        highlightthickness=0,
        command=lambda: aStar_solve_only(),
        relief="flat"
    )
    ...
    def aStar_solve_only():
        ...
        frame.text_1.insert("1.0", "Giải với A*:\n")
    ...
    def view():
        ...
        start_time = time.perf_counter()
        path, info, dataMap = aStar_and_generation_data(root)
        # rs = AStar(root)
        end_time = time.perf_counter()
        elapsed_time = end_time - start_time # Thời gian tính toán
