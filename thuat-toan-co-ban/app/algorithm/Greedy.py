...
def Greedy(start):
    ...
    while openSet:
        ...
        if check_is_goal(current):
            print("success")
            return reconstruct_path(cameFrom, current_sig, visited_count=visited_count)

        neighbors = get_neighbors(current)

        for neighbor in neighbors:
            neighbor_sig = neighbor.tobytes()
            tentative_gScore = gScore[current_sig] + 0 # <- Khac cho nay
...
def greedy_and_generation_data(root):
    path, info, dataMap = Greedy_2(root)
    print("info: ", info)
    print("path: ", path)
