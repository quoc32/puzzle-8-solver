...
# **Gọi hàm với file input**
file_path = "pygame_demo/data.txt"
    ...
    for node in nodes:
        if node in path_set:
            if (node == str(info_dict["root"])) or (node == "123456780"):
                dot.node(node, shape="square", style="filled", fillcolor="green")
                continue
            dot.node(node, shape="square", style="filled", fillcolor="red")
        else:
            dot.node(node, shape="square")
